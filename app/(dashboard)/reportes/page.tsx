...
export default function ReportesPage() {
          ...
          resultados.innerHTML = clientes.map(cliente => `
            <tr class="cliente-seleccionado hover:bg-gray-50 cursor-pointer" 
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                <span class="px-2 py-1 text-xs font-semibold rounded-full ${cliente.activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
  ...
  // Modificación aquí: Mostrar modal de búsqueda si es reporte por cliente y no hay RTN
  if (tipo === "cliente" && !rtn) {
    const cliente = await mostrarModalBusquedaClientes();
    if (!cliente) return;
    rtn = cliente.rtn;
  ...
  setModalPreview({
    ...
    fechaInicio,
    fechaFinal,
    filtroNombre: tipo === "cliente" ? `Cliente RTN: ${rtn}` : 
                 tipo === "usuario" ? `Técnico: ${id}` : 
                 tipo === "ventas" ? `Técnico: ${usuario}` : 
                 "Reporte General",
                          ...
                          <tr key={index} className="hover:bg-gray-50">
                            ...
                            {modalPreview.tipo === "usuario" && (
                              <>
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{bitacora.id}</td>
                                <td className="px-4 py-4 text-sm text-gray-900 max-w-32 truncate" title={bitacora.cliente}>{bitacora.cliente}</td>
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{bitacora.horas}</td>
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{bitacora.tipo_horas}</td>
